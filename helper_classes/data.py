...
def charLimit(
    big_message,
):  # Splits the message into 2000 character segments for Discord character limit
...
def logError(error):
    with open("E:/guac_data/error_log.txt", "a") as f:
        f.write(f"[{time.strftime('%d/%m/%Y, %H:%M:%S')}] {error}\n\n")

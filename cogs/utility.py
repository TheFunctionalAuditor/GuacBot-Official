...
from helper_classes.data import Jason, logError
...
class Utility(
    commands.Cog
):  # Commands for general utility purposes (usually stats or other info gathering stuff)
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @utility.command(
        description="Shows the avatar of the given member (or user if not given)."
    )  # Slash command for showing the avatar of a given member (or the user if none given)
    async def avatar(self, ctx, member: discord.Member):
        if member is None:
            member = ctx.author
        await ctx.respond(f"{member.display_avatar.url}", ephemeral=True)

    @utility.command(
        description="Returns an image of the given custom emoji."
    )  # Slash command for returning an image of the given custom emoji
    ...
    @utility.command(
        description="Shows total + human + online/DND member count."
    )  # Slash command for showing how many members are in the server
    @discord.guild_only()
    async def role_call(self, ctx):
        ...
        embed = discord.Embed(
            title="Rolecall",
            description=f"{ctx.guild.member_count} members including me :)",
        )
        ...
        for member in ctx.guild.members:
            if not member.bot:
                not_bots += 1
        embed.add_field(name='"Human" Member(s):', value=str(not_bots), inline=False)

        # Online members
        for member in ctx.guild.members:
            if (
                str(member.status) == "online" or str(member.status) == "dnd"
            ) and not member.bot:
                online += 1
        embed.add_field(
            name='"Human" Member(s) Online/DND:', value=str(online), inline=False
        )

        await ctx.respond(embed=embed)

    @utility.command(
        description="Returns server data on the user."
    )  # Slash command for returning server data on the user
    @discord.guild_only()
    async def profile(self, ctx):
        ...
        embed = discord.Embed(
            ...
            url="https://github.com/guacamolefather?tab=repositories",
        )
        ...
        embed.add_field(
            ...
            value=member.created_at.strftime("%d/%m/%Y at %H:%M:%S"),
            inline=False,
        )
        ...
        embed.add_field(
            name="Channels:",
            value=f"Has access to {has_counter} out of {is_counter} channels.",
            inline=False,
        )
        ...
        if is_banned:
            embed.add_field(
                name="Reactions status:", value=f"Banned {how}", inline=False
            )
        else:
            embed.add_field(
                name="Reactions status:",
                value="Available! Feel free to talk to me (GuacBot) :)",
                inline=False,
            )
        ...
        embed.add_field(
            ...
            value="This is the regular profile command. For the admin version, use /profile from the admin section.",
            inline=False,
        )
    ...
    )  # Slash command for showing info about a role
    @discord.guild_only()
    async def role_info(self, ctx, role: discord.Role):
        ...
        embed.add_field(
            name="Created on:",
            value=str(role.created_at.strftime("%d/%m/%Y at %H:%M:%S")),
            inline=False,
        )
        embed.add_field(
            name="Members with this role:", value=str(len(role.members)), inline=False
        )

...
from helper_classes.data import Jason, charLimit, logError
...
def llmMessage(message, personality, sending_message):
    ...
    for loop_personality in brain_data.keys():
        if loop_personality != "lookup":
            brain_data[loop_personality]["mod_memory"].append(
                {
                    "role": "user",
                    "content": f"{current_time} - {message.guild} server in the #{message.channel} channel - {name} says: {message.content}",
                }
            )  # Add most recent message to memory

    if sending_message:
        messages = brain_data[personality][
            "core_memory"
        ]  # Add personality specific memory
        messages = (
            messages + brain_data[personality]["mod_memory"]
        )  # Combine personality memory with contemporary memory

        try:
            model_path = (
                "E:/guac_data/llm_files/"
                + brain_data[personality]["model_info"]["model"]
            )
        ...
        for loop_personality in brain_data.keys():
            if loop_personality != "lookup":
                if loop_personality == personality:
                    brain_data[personality]["mod_memory"].append(
                        {"role": "assistant", "content": content}
                    )  # Add Guac/Salsa's response to their own contemporary memory
                else:
                    brain_data[loop_personality]["mod_memory"].append(
                        {
                            "role": "user",
                            "content": f"{current_time} - {message.guild} server in the #{message.channel} channel - {personality} says: {content}",
                        }
                    )  # Add Guac/Salsa's response to other AIs' contemporary memory

    for loop_personality in brain_data.keys():
        if loop_personality != "lookup":
            mod_len = len(brain_data[loop_personality]["mod_memory"])
            if mod_len > 80:  # If memory is too long, remove the oldest messages
                for i in range(mod_len - 80):
...
class Reaction(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if (
            "not now, guac" == lower_message and message.author.id == 409445517509001216
        ):  # For Guac to be told to stop reacting temporarily
        ...
        if (
            bot_data["reaction_cog"]["wait_until"] > time.time()
        ):  # If Guac was told to stop reacting temporarily
            return
        ...
        if (
            ...
            in bot_data["reaction_cog"]["global_blacklist"].keys()
        ):  # If the author is globally blacklisted
            return

        if message.guild is None:
            return

        if not server_data[str(message.guild.id)]["reaction_cog"][
            "reactions"
        ]:  # If reactions are disabled in the server
            return

        if (
            message.author.bot
            and not server_data[str(message.guild.id)]["reaction_cog"]["bot_reactions"]
        ):  # If the author is a bot and bot reactions are disabled in the server
            return

        if (
            message.guild.id in bot_data["reaction_cog"]["server_blacklist"]
            or message.author.id
            in server_data[str(message.guild.id)]["reaction_cog"]["blacklist"]
        ):  # If the server or author is blacklisted
            return
        ...
        ):  # If Guac is mentioned in the message (but not in a reply or along with everyone)
            await message.channel.send(
                "You can use my slash commands with necessary permissions, use /invite to invite me to your own server, or use this link to join the support server: https://discord.gg/2kgZazXN68"
            )
            return
        elif (
            self.bot.user.mentioned_in(message)
        ...
        if (
            (
                ...
                or (
                    random.randint(1, 50 - bot_data["reaction_cog"]["random_limiter"])
                    == 1
                )
            )
            and str(message.guild.id)
            in bot_data["reaction_cog"]["server_whitelist"].keys()
        ):  # If the message is addressed to GuacBot or SalsAI and the server is AI whitelisted
            if (
                bot_data["reaction_cog"]["random_limiter"] == 49
                or not personality_mentioned
            ):
            ...
            if personality_mentioned:
                async with message.channel.typing():
                    personality_message = charLimit(
                        llmMessage(message, triggered, True)
                    )
                ...
                if (
                    personality_message[0]
                    == "Sorry, my AI capabilities are currently offline. Please try again later."
                ):
                    return
        ...
        punc = r"""!()-[]{};:'"\, <>./?@#$%^&*_~"""
        unpunc_lower_message = (
            message.content.lower()
        )  # Remove punctuation from message (unPunctuatedMessage)
        ...
        for trigger in speech_dict[
            "triggers"
        ].keys():  # Check for triggers in message and add reactions to list
            if trigger in unpunc_lower_message:
                if (
                    unpunc_lower_message == trigger
                    or (
                        unpunc_lower_message.startswith(trigger)
                        and f"{trigger} " in unpunc_lower_message
                    )
                    or (
                        unpunc_lower_message.endswith(trigger)
                        and f" {trigger}" in unpunc_lower_message
                    )
                ):
                    reactions.append(
                        speech_dict["reactions"][speech_dict["triggers"][trigger]]
                    )
        ...
        if "i’m " in message.content.lower():
            if "I’m" in message.content:
                reactions.append(
                    'Hello "' + message.content.split("I’m ")[1] + "\", I'm GuacBot!"
                )
            else:
                reactions.append(
                    'Hello "' + message.content.split("i’m ")[1] + "\", I'm GuacBot!"
            ...
            if "I am" in message.content:
                reactions.append(
                    'Hello "' + message.content.split("I am ")[1] + "\", I'm GuacBot!"
                )
            else:
                reactions.append(
                    'Hello "' + message.content.split("i am ")[1] + "\", I'm GuacBot!"
        ...
        if (
            "micolash" in unpunc_lower_message
            or "kos" in unpunc_lower_message
            or "bloodborne" in unpunc_lower_message
        ):
            quotes = [
        ...
        if len(reactions) > 7:
            ...
            jason.updateBotData(bot_data)
            await message.channel.send(
                f"{message.author.mention}, you have been deemed up to no good and are now globally banned from my services. DM @guacamolefather if you believe this is a mistake."
            )

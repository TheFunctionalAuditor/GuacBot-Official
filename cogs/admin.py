...
from helper_classes.data import Jason
...
class Admin(commands.Cog):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @admin.command(
        description="Gets rid of a specified amount of messages."
    )  # Slash command for clearing messages
    @discord.default_permissions(
        manage_messages=True
    )  # User needs to have manage messages permission
    ...
    @admin.command(
        description="Steals the given custom emoji for the server."
    )  # Slash command for stealing an emoji
    @discord.default_permissions(
        manage_emojis=True
    )  # Only full admins can use this command
    @discord.guild_only()  # Only works in guilds
    ...
    async def profile(self, ctx, member: discord.Member):
        ...
        # Member's join date:
        joined = (
            member.joined_at.strftime("%d/%m/%Y at %H:%M:%S")
            if member.joined_at
            else "I have no clue."
        )
        embed.add_field(
    ...
    @admin.command(
        description="Counts the amount of people with certain roles."
    )  # Slash command for counting the amount of people with certain roles
    @discord.default_permissions(
        manage_roles=True
    )  # Only full admins can use this command
    @discord.guild_only()  # Only works in guilds
    async def role_count(self, ctx):
        ...
        for role in ctx.guild.roles:
            ...
            embed.add_field(
                name=f"{role} - created on {role.created_at.strftime('%d/%m/%Y at %H:%M:%S')}:",
                value=f"{with_role}: ({members_with_role})",
                inline=False,
            )
            with_role = 0
            members_with_role = ""

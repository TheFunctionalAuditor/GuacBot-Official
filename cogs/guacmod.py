...
class GuacMod(commands.Cog):  # Commands for moderating GuacBot
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @guac.command(
        description="Shows the json data for the server."
    )  # Slash command for showing the json data for the server
    ...
    @guac.command(
        description="Blacklists the specified member from reactions."
    )  # Slash command for server blacklisting a member from reactions
    @discord.default_permissions(
        administrator=True
    )  # Only full admins can use this command
    @discord.guild_only()
    async def blacklist(self, ctx, member: discord.Member):
        ...
        if member.id in server_data[str(ctx.guild.id)]["reaction_cog"]["blacklist"]:
            await ctx.respond(
                f"{member.display_name} already blacklisted for reactions."
            )
            return
    ...
    @guac.command(
        description="Unblacklists the specified member from reactions."
    )  # Slash command for server unblacklisting a member from reactions
    @discord.default_permissions(
        administrator=True
    )  # Only full admins can use this command
    @discord.guild_only()
    async def unblacklist(self, ctx, member: discord.Member):
        ...
        if member.id not in indexing_purposes:
            await ctx.respond(
                f"{member.display_name} already not blacklisted for reactions."
            )
            return
        server_data[str(ctx.guild.id)]["reaction_cog"]["blacklist"].pop(
            indexing_purposes.index(member.id)
        )
        jason.updateServerData(server_data)
    ...
    async def bot_reaction_switch(self, ctx):
        server_data = jason.fetchServerData()
        duplication_purposes = server_data[str(ctx.guild.id)]["reaction_cog"][
            "reactions"
        ]
        server_data[str(ctx.guild.id)]["reaction_cog"][
            "reactions"
        ] = not duplication_purposes
        jason.updateServerData(server_data)
    ...
    async def economy_switch(self, ctx):
        ...
        server_data[str(ctx.guild.id)]["economy_cog"][
            "economy"
        ] = not duplication_purposes
        jason.updateServerData(server_data)

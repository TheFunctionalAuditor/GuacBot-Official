...
from helper_classes.data import Jason, charLimit, is_it_me, logError
...
class Owner(commands.Cog):  # Commands just for me :)
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ HQ COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @commands.command(
        hidden=True, description="Changes Guac's status."
    )  # Command for changing Guac's status
    ...
    async def change_status(self, ctx, activity: str, *, status: str):
        ...
        # Setting "Streaming" status
            ...
            await self.bot.change_presence(
                activity=discord.Streaming(
                    name=status, url="https://www.twitch.tv/thememesareallreal"
                )
            )
        ...
        # Setting "Playing" status
            ...
            await self.bot.change_presence(
                activity=discord.Game(f"{activity} {status}")
            )
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ DATA COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @commands.command(
        hidden=True, description="Globally Guac blacklists the specified member."
    )  # Command for globally blacklisting a member
    @commands.check(is_it_me)  # Only dad can use this command
    async def global_blacklist(self, ctx, member: discord.Member):
        bot_data = jason.fetchBotData()
        global_blacklist = bot_data["reaction_cog"]["global_blacklist"]

        if member.id in global_blacklist:
            await ctx.respond("Member already on the global blacklist.", ephemeral=True)
            return

        global_blacklist[f"{member.id}"] = member.name
        jason.updateBotData(bot_data)
        await ctx.respond(f"{member.name} now on Guac global blacklist.")

    @commands.command(
        hidden=True, description="Globally Guac unblacklists the specified member."
    )  # Command for globally unblacklisting a member
    @commands.check(is_it_me)  # Only dad can use this command
    async def global_unblacklist(self, ctx, member: discord.Member):
        ...
        if member.id in global_blacklist:
            del global_blacklist[f"{member.id}"]
        else:
        ...
        jason.updateBotData(bot_data)
        await ctx.respond(
            f"{member.name} bailed from Guac global blacklist :rolling_eyes:"
        )

    @commands.command(
        hidden=True, description="Shows global blacklist."
    )  # Command for showing the global blacklist
    @commands.check(is_it_me)  # Only dad can use this command
    ...
    @commands.command(
        hidden=True, description="Locates the member based on their ID."
    )  # Command for locating a member based on their ID")
    @commands.check(is_it_me)  # Only dad can use this command
        ...
        try:
            ...
            if not isinstance(e, discord.NotFound):
                if not isinstance(e, ValueError):
                    ...
                    await ctx.respond("Invalid member ID.", ephemeral=True)
            else:
                await ctx.respond("Member not found.", ephemeral=True)

    @commands.command(
        hidden=True, description="Pings Guac."
    )  # Command for pinging the bot to check latency
    @commands.check(is_it_me)  # Only dad can use this command
    async def ping(self, ctx):
        await ctx.respond(
            f"Latency: {round(self.bot.latency * 1000)}ms.", ephemeral=True
        )

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ BRAIN COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @commands.command(
        hidden=True, description="Empties GuacBot's short-term memory."
    )  # Command for emptying GuacBot's short-term memory
    @commands.check(is_it_me)  # Only dad can use this command
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ PERSONAL COMMANDS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    @commands.command(
        hidden=True, description="Adds something to the to-do list."
    )  # Command for adding something to the to-do list
    @commands.check(is_it_me)  # Only dad can use this command
    ...
    @commands.command(
        hidden=True, description="fetches pending to-do list items."
    )  # Command for collecting unreacted messages in #guacbot-to-do
    @commands.check(is_it_me)  # Only dad can use this command
    ...
    @commands.command(
        hidden=True,
        description="Updates the patch notes + version and sends them to the announcements channel.",
    )  # Command for updating the patch notes and version
    @commands.check(is_it_me)  # Only dad can use this command
    async def patch(self, ctx, patch_type: str, *, patch_info: str):
        ...
        if patch_type.startswith("major"):
            ...
            split_version = last_version.split(".")
            new_version = (
                f"{split_version[0]}.{split_version[1]}.{int(split_version[2]) + 1}"
            )
    ...
    @commands.command(
        hidden=True, description="Adds a new nickname to the list."
    )  # Command for adding a new nickname to the list
    @commands.check(is_it_me)  # Only dad can use this command
